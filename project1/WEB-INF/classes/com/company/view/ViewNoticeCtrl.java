...
@WebServlet("/ViewNoticeCtrl")
public class ViewNoticeCtrl extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(rs.next()) { 
				...
				noti.setResdate(rs.getDate("resdate"));
				noti.setVisited(rs.getInt("visited"));
			} else {
				response.sendRedirect("NoticeListCtrl");
			}
			request.setAttribute("noti", noti);
			RequestDispatcher view = request.getRequestDispatcher("viewNotice.jsp");
