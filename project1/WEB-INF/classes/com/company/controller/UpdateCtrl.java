...
public class UpdateCtrl extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			sql = "update membership set m_name=?, m_email=?, m_tel=?, m_zip=?, m_addr1=?, m_addr2=?, m_job=? where m_id=?";
			...
			if(cnt == 0) { 
				out.println("<script>alert('상품 수정이 거부되었습니다'); location.href='mypage.jsp';</script>");
			} else {
